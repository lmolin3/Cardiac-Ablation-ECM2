...
//
// Description:  This example extends tutorial_emory_03.cpp to demonstrate the use of
//           Partial Assembly (PA) for the linear elasticity problem.
//           When partial assembly is enabled, the system matrix is not
//           explicitly formed, and a Low Order Refined version of BoomerAMG 
//           is used as a preconditioner for the PCG solver.
//
// Compile with: make tutorial_emory_03_pa
//
// Sample runs:  
// No refinement, third order elements with traditional full assembly (equivalent to tutorial_emory_03):
//                mpirun -np 8 tutorial_emory_03_pa -rs 0 -o 3 -no-pa -of ./Output/Tutorial02/FA
//
// No refinement, fourth order elements with partial assembly:
//                mpirun -np 8 tutorial_emory_03_pa -rs 0 -o 3 -pa -of ./Output/Tutorial02/PA
//
// For the tutorial, run the examples with flag 
//          -of /home/shared/<folder-path>,
// to ensure that the results will eb directly available in the folder mounted 
...
int main(int argc, char *argv[])
{
   ...
   int order = 1;
   bool partial_assembly = false; // partial assembly
   bool byNodes = false;    
   // Output parameters
   ...
   args.AddOption(&partial_assembly,  "-pa", "--partial-assembly", "-no-pa",
                  "--no-partial-assembly", "Enable Partial Assembly.");
   args.AddOption(&outfolder, "-of", "--output-folder", "Output folder.");
   args.ParseCheck();

   // Set to true if using partial assembly (because internally PA for elasticity uses byNODES)
   byNodes = partial_assembly; 
   ...
   Ordering::Type ordering = byNodes ? Ordering::byNODES : Ordering::byVDIM;
   ...
   if (Mpi::Root())
   {
      out << "Number of finite element unknowns: " << size << std::endl;
   }
   ...
   if (Mpi::Root()) { out << "Assembling matrix ... " << std::flush; }
   a->Assemble();
   ...
   {
      out << "done." << std::endl;
      out << "Size of linear system: " << fespace->GlobalTrueVSize() << std::endl;
   }
   ...
   /////////////////////////////////////////////////////////////////////////////
   //------     7. Define solver/preconditioner 
   /////////////////////////////////////////////////////////////////////////////

   if (Mpi::Root())
   {
      out << "Setting up solver..." << std::flush;
   }


   //<--- Define a parallel PCG solver for A X = B with the BoomerAMG
   //     preconditioner from hypre.
   HypreBoomerAMG *prec = nullptr;
   ParLORDiscretization *lor = nullptr;
   if (partial_assembly)
   {
      // LOR AMG preconditioner for partial assembly
      lor = new ParLORDiscretization(*a, ess_tdof_list);
   ...
   prec->SetSystemsOptions(dim, byNodes);
