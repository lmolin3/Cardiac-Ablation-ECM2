...
void NavierUnsteadySolver::Step(double &time, double dt, int current_step)
{
   ...
   A = Add(1.0, *NL, C_visccoeff.constant, *K);   // A = kin_vis K + NL
   C = Add(1.0, *A, C_bdfcoeff.constant, *M);      // C = alpha/dt M + A
   
   auto Ae = A->EliminateRowsCols(vel_ess_tdof);
   delete Ae;
   ...
   *sigmaM *= C_bdfcoeff.constant;
   ...
   G->Mult(*p_pred,*tmp1);          //         G p_pred = Gp   --> stored to be reused in Velocity correction
   H1->Mult(*tmp1,*tmp2);           //       H G p_pred = tmp2
   A->Mult(*tmp2,*tmp1);            //     A H G p_pred = tmp1
   H1->Mult(*tmp1,*tmp2);           //   H A H G p_pred = tmp2
   D->Mult(*tmp2,*rhs_p2);          // D H A H G p_pred = tmp1
   ...
   *p = *p_pred;
   ...
   // Assemble rhs           rhs_v1 - G p 
   rhs_v2->Set(1.0,*rhs_v1);
   G->AddMult(*p,*rhs_v2,-1.0); 

   // Solve current iteration.
   invC->Mult(*rhs_v2,*u);  
   iter_v2solve = invC->GetNumIterations();
   res_v2solve = invC->GetFinalNorm();
   MFEM_VERIFY(invC->GetConverged(), "Velocity correction step did not converge. Aborting!");
