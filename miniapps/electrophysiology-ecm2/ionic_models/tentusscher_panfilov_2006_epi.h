...
//     cellml/tp06_epi.ode  ->  gotranx ode2c  ->  tp06_epi_generated.h
// Regenerate with:
//     python3 gotranx_to_mfem.py --config models.json
//
// ten Tusscher-Panfilov 2006 human ventricular epicardial cell model.
...
namespace mfem
{
    namespace electrophysiology
    {
        ...
        /** @brief ten Tusscher-Panfilov 2006 human ventricular epicardial cell model. */
        class TP06Epi : public EPModelBase
        {
        public:
            TP06Epi() : EPModelBase()
            {
                ...
                dimensionless = Kernel::dimensionless;   // physiological mV
            ...
            std::string GetName() const override { return "ten Tusscher-Panfilov 2006 (epicardial)"; }

            int GetPotentialIndex() const override { return Kernel::potential_idx; }

            /// TP06 resolves [Ca2+]i explicitly, so it can drive a contraction model.
            ...
            void init_state_values(double* states){
                /*
                fCass=0.9953, f=0.7888, f2=0.9755, r=2.42e-08, s=0.999998, Ca_i=0.000126, Na_i=8.604, h=0.7444, j=0.7045, m=0.00172, Xr1=0.00621, Xr2=0.4712, Xs=0.0095, d=3.373e-05, R_prime=0.9073, K_i=136.89, V=-85.23, Ca_SR=3.64, Ca_ss=0.00036
                */
                states[0] = 0.9953;
                states[1] = 0.7888;
                states[2] = 0.9755;
            ...
            struct Kernel : IonicKernelDefaults
            {
                ...
                MFEM_HOST_DEVICE static void rhs(const double t, const double *__restrict states, const double *__restrict parameters, double* values){
                    ...
                    const double s_inf = 1.0/(exp((V + 20.0)/5.0) + 1.0);
                    ...
                    const double tau_s = (85.0*exp((-pow(V + 45.0, 2.0))/320.0) + 5.0/(exp((V - 1*20.0)/5.0) + 1.0)) + 3.0;
