...
//     land_2017.ode  ->  gotranx ode2c  ->  land_2017_generated.h
// Regenerate with:
//     python3 gotranx_to_mfem.py --config models.json
//
// Land et al. 2017 human cardiomyocyte active contraction model.
...
namespace mfem
{
    namespace electrophysiology
    {
        ...
        /** @brief Land et al. 2017 human cardiomyocyte active contraction model. */
        class Land17Model : public ContractionModelBase
        {
        public:
            Land17Model() : ContractionModelBase()
            {
                NUM_STATES = Kernel::nstates;
                NUM_PARAMS = Kernel::nparams;
                NUM_MONITORED = Kernel::nmonitored;

                // Land 2017 has no membrane potential and no stimulus of its own: it is
                // driven entirely by the calcium transient handed to it by the EP model.
                potential_idx = -1;

                MFEM_ASSERT(parameter_index("cai") == Kernel::calcium_param_idx, "cai index mismatch");
                MFEM_ASSERT(monitor_index("Ta") == Kernel::active_tension_monitor_idx, "Ta index mismatch");
            }

            std::string GetName() const override { return "Land et al. 2017 contraction"; }

            bool RequiresCalcium() const override { return true; }

            int GetActiveTensionIndex() const override
            { return Kernel::active_tension_monitor_idx; }

            /// Index of the calcium input within the parameter array.
            int GetCalciumParameterIndex() const { return Kernel::calcium_param_idx; }

            void AdvanceODE(const ContractionStepArgs &args) const override
            {
                ContractionAdvance<Kernel>(args);
            }
            ...
            {
                static constexpr int nstates = 7;
                static constexpr int nparams = 26;
                static constexpr int nmonitored = 33;

                static constexpr int calcium_param_idx = 5;   // "cai"
                static constexpr int active_tension_monitor_idx = 30;   // "Ta"
                static constexpr int lmbda_param_idx = 16;   // "lmbda"
                static constexpr int dLambda_param_idx = 7;   // "dLambda"
