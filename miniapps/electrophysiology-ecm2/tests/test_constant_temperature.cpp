...
// Sample runs:
//
//   ./test_constant_temperature -tf 100 -of ./Output/ConstantTemperature
//
// GPU:
//   ./test_constant_temperature -tf 100 -dev cuda -of ./Output/ConstantTemperature
//
//
...
real_t CelsiusToKelvin(real_t T_celsius)
{
...
int main(int argc, char *argv[])
{
    ...
    real_t T_tissue = 37.0; // constant tissue temperature in degree Celsius
    ...
    args.AddOption(&T_tissue, "-T", "--tissue-temperature",
                   "Constant tissue temperature in degree Celsius");
    // Output
    ...
    *nodes -= 2.5;
    ...
    //<--- 5.4 For temperature/damage dependent models, set the temperature GridFunction

    ParGridFunction temperature_gf;
    auto *temperature_coeff = new ConstantCoefficient(CelsiusToKelvin(T_tissue)); // K
    ...
    // Both CORNER and PLANE_WAVE are functions of x only: fixed spatial masks whose
    // temporal gating is done inside the ionic model. Strategy (a) -- see
    ...
    for (int step = 0; !last_step; ++step)
    {
        ...
        if (ep_ctx.has_thermal_dependency)
        {
            ...
            temperature_gf.ProjectCoefficient(*temperature_coeff);
        }
