...
void conductivity_function(const Vector &x,  real_t t, DenseMatrix &Sigma)
{
    ...
    Sigma = 0.0;
    Sigma(0, 0) = ep_ctx.matrix_factor * (ep_ctx.sigma_min + (ep_ctx.sigma - ep_ctx.sigma_min) * (1.0 - damage_value));
    Sigma(1, 1) = ep_ctx.matrix_factor * (ep_ctx.sigma_min + (ep_ctx.sigma - ep_ctx.sigma_min) * (1.0 - damage_value));
    if (x.Size() > 2)
    {
        Sigma(2, 2) = ep_ctx.matrix_factor * ep_ctx.matrix_factor * (ep_ctx.sigma_min + (ep_ctx.sigma - ep_ctx.sigma_min) * (1.0 - damage_value));
    }
}
...
// Define a damage function (for damage-dependent models)
real_t damage_function(const Vector &x, real_t t)
{
    // Circle in the center, radius 0.5
    real_t xc = 0.0;
    real_t yc = 0.0;
    real_t zc = 0.0;

    real_t R = 0.5;          // Radius of the damaged region
    real_t sharpness = 20;  //  transition
    real_t r2 = (x(0) - xc) * (x(0) - xc) + (x(1) - yc) * (x(1) - yc);
    if (x.Size() > 2)
    {
        r2 += (x(2) - zc) * (x(2) - zc); // Add z-component for 3D
