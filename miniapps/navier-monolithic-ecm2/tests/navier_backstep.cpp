...
// Sample run:
// 1. 
// mpirun -np 4 ./navier-backstep-monolithic -d 2 -rs 0 -rp 0 -ou 2 -op 1 -dt 1e-3 -tf 1e-1 -tp 1e-2 -kv 0.1 -u 1.0 --preconditioner 0 --schur-preconditioner 5
...
struct s_NavierContext // Navier Stokes params
{
   ...
   int schur_pc_type = 1; // 0: Pressure Mass, 1: Pressure Laplacian, 2: PCD, 3: Cahouet-Chabard, 4: LSC, 5: Approximate Inverse   int correction_order = 1; // Correction order for High-Order Yosida
} NS_ctx;
