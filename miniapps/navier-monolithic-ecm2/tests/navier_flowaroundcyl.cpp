...
// Sample run:
//
// 1. Yosida preconditioner + DiscreteApproximateLaplacian Schur complement preconditioner 
// mpirun -np 4 ./navier-flowaroundcyl-monolithic -d 2 -rs 0 -ou 2 -op 1 -dt 1e-3 -tf 1e-1 -kv 1.0 -re 10 --preconditioner 4 --schur-preconditioner 5
//
// 2. Mass lumping
// mpirun -np 4 ./navier-flowaroundcyl-monolithic -d 2 -rs 0 -ou 2 -op 1 -dt 1e-3 -tf 1e-1 -kv 1.0 -re 10 --preconditioner 4 --schur-preconditioner 5 --mass-lumping
//
// 3. Stiff strain
// mpirun -np 4 ./navier-flowaroundcyl-monolithic -d 2 -rs 0 -ou 2 -op 1 -dt 1e-3 -tf 1e-1 -kv 1.0 -re 10 --preconditioner 4 --schur-preconditioner 5 --stiff-strain
//
...
struct s_NavierContext // Navier Stokes params
{
   ...
   double kinvis = 1.0;
   double re = 100;
   double Umax =  re * kinvis / Mesh_ctx.D;
   double dt = 1e-3;
...
int main(int argc, char *argv[])
{
   ...
   ///////////////////////////////////////////////////////////////////////////////////////////////
    ...
    case 2:
    {
        mesh = Mesh::LoadFromFile("./Mesh/flow_around_cyl_2D.msh");
        break;
    }
    case 3:
    {
        mesh = Mesh::LoadFromFile("./Mesh/flow_around_sphere_3D.msh");
        break;
    }
    default:
        break;
   ...
   NS_ctx.Umax = NS_ctx.re * NS_ctx.kinvis / Mesh_ctx.D;  // From Re = U D / kinvis
   ...
   // NOTE: step should start from 1, otherwise there's gonna be errors with HypreSolver (12) in the DHG operator BoomerAMG
...
void inflow(const Vector &x, double t, Vector &u)
{
   ...
   {
      double zi = x[2];
      u(0) = preload*( 16.0 * NS_ctx.Umax * yi * zi / std::pow(Mesh_ctx.D,2.0) ) * ( 1.0 - yi/Mesh_ctx.D) * ( 1.0 - zi/Mesh_ctx.D);
      u(2) = 0.0;
   }
   else
   {
      u(0) = preload*( 4.0 * NS_ctx.Umax * yi / Mesh_ctx.D) * ( 1.0 - yi/Mesh_ctx.D);
   }
