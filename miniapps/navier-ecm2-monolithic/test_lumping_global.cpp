...
//
// Run with:
// mpirun -np 4 ./test_lumping_global -d 2 -e 1 -n 10 -ou 2 -f 1 --output-folder ./Output/TestLumping/ -pm 
...
void x_lin(const Vector &X, Vector &v);
void x_const(const Vector &X, Vector &v);
...
int main(int argc, char *argv[])
{
   ...
   if (pmesh->GetMyRank() == 0)
   {
    ...
    VectorFunctionCoefficient* x_fun = nullptr;
    switch (fun)
    {
    case 1:
    {
        x_fun = new VectorFunctionCoefficient(dim, x_const);
        break;
    }
    case 2:
    {
        x_fun = new VectorFunctionCoefficient(dim, x_lin);
        break;
    }
    default:
        break;
