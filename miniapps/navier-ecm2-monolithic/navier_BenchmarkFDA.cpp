...
//           |\    |              \_________________|               |
// Parabolic |_|   |                                                |  Traction free (outflow)
//  inflow   |_|   |               _________________                |
//           |/    |______________/                 |_______________|
//                                      no slip
//
...
//
// Run with:
// mpirun -np 4 ./navier_ecm2_benchmarkFDA_mono -d 2 -rs 0 -rp 0 -ou 2 -op 1 -dt 1e-3 -tf 1e-1 -tp 1e-2 -fr 5E6 -u 1.0 --gamma 1.0 --verbose --paraview --output-folder ./Output/BenchmarkFDA/2D/Test
//
...
struct s_NavierContext // Navier Stokes params
{
   int uorder = 2;
   int porder = 1;
   double rho = 1056;                   // kg/m^3
   double kinvis = 0.0035 / rho;        // m^2/s
   double flow_rate = 5.21E6;           // m^3/s
   double inlet_area = 0.012;           // m^2
   double Umax =  0.1; // m/s
   double dt = 1e-3;
   ...
   bool verbose = true;
   bool paraview = false;
   const char *outfolder = "./Output/BenchmarkFDA/2D/Test/";
...
int main(int argc, char *argv[])
{
   ...
   args.AddOption(&NS_ctx.flow_rate, "-fr", "--flow-rate", "Flow Rate.");
   ...
   case 2:
   {
      mesh = Mesh::LoadFromFile("./Mesh/BenchmarkFDA2D.msh");
      break;
   }
   case 3:
   {
      mesh = Mesh::LoadFromFile("./Mesh/BenchmarkFDA3D.msh");
      break;
   }
   default:
      break;
   ...
   {
      mfem::out << "Kinematic viscosity: " << NS_ctx.kinvis << " [m^2/s] " << std::endl;
      mfem::out << "Flow Rate: " << NS_ctx.flow_rate << " [m^3/s] " << std::endl;
      mfem::out << "Max velocity: " << NS_ctx.Umax << " [m/s] " << std::endl;
      mfem::out << std::endl;
   }
...
void inflow(const Vector &x, double t, Vector &u)
{
   ...
   {  
      double rho = yi/Mesh_ctx.R;
      u(0) = preload * NS_ctx.Umax * ( 1.0 - std::pow(rho,2.0) ) ;
   }
