// Sample run: 
//             mpirun -np 2 ./test-transfer-2d -o 1 -of ./Output/Transfer2D
...
int main(int argc, char *argv[])
{
   ...
   ParFiniteElementSpace fes_solid(solid_submesh.get(), &fec);
   ParFiniteElementSpace fes_solid_grad(solid_submesh.get(), &fec, sdim);

   ParFiniteElementSpace fes_fluid(fluid_submesh.get(), &fec);
   ParFiniteElementSpace fes_fluid_grad(fluid_submesh.get(), &fec, sdim);

   ParGridFunction temperature_solid_gf(&fes_solid); temperature_solid_gf = 0.0;
   ParGridFunction grad_solid_gf(&fes_solid_grad); grad_solid_gf = 0.0;
   ParGridFunction temperature_solid_gf_copy(&fes_solid); temperature_solid_gf_copy = 0.0;

   ParGridFunction temperature_fluid_gf(&fes_fluid);    temperature_fluid_gf = 0.0;
   ParGridFunction grad_fluid_exact_gf(&fes_fluid_grad); grad_fluid_exact_gf = 0.0;
   ...
   InterfaceTransfer finder_fluid_to_solid_hybrid(temperature_fluid_gf, temperature_solid_gf, fluid_solid_interface_marker, TransferBackend::Hybrid, parent_mesh.GetComm());
   ...
   InterfaceTransfer finder_fluid_to_solid_gslib(temperature_fluid_gf, temperature_solid_gf, fluid_solid_interface_marker, TransferBackend::GSLIB, parent_mesh.GetComm());
   ...
   auto solid_grad_func = [&](ElementTransformation &Tr, const IntegrationPoint &ip, Vector &qoi_loc)
   {
      temperature_solid_gf.GetGradient(Tr, qoi_loc);
   };

   auto fluid_grad_func = [&](ElementTransformation &Tr, const IntegrationPoint &ip, Vector &qoi_loc)
   {
   ...
   if (paraview)
   {
      ...
      paraview_dc_fluid.RegisterField("Gradient-exact", &grad_fluid_exact_gf); 
      ...
      if(order > 1 )
      {
         ...
         paraview_dc_solid.SetLevelsOfDetail(order);
      }
      ...
      paraview_dc_solid.RegisterField("Gradient", &grad_solid_gf);
   }
   ...
   // Define Temperature analytic field in the solid domain
   // f(x,y) = x^2
   ...
   grad_fluid_exact_gf.ProjectCoefficient(grad_Tfunc);

   // Export before transfer
   if ( paraview )
   {
      paraview_dc_solid.SetCycle(0); paraview_dc_solid.SetTime(0.0); paraview_dc_solid.Save();
   ...
   chrono.Clear();
   chrono.Start();
   finder_fluid_to_solid_gslib.InterpolateQoI(fluid_grad_func, grad_solid_gf);
   chrono.Stop();
   ...
   if (paraview)
   {
      paraview_dc_solid.SetCycle(1); paraview_dc_solid.SetTime(0.1); paraview_dc_solid.Save();
