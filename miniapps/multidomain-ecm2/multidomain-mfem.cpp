...
int main(int argc, char *argv[])
{
   ...
   ess_tdofs.Sort();
   ess_tdofs.Unique();
   ConvectionDiffusionTDO cd_tdo(fes_cylinder, ess_tdofs, 0.0, 1.0);
   ...
   for (int ti = 1; !last_step; ti++)
   {
      ...
      if (paraview && (ti % vis_steps == 0))
      {
         paraview_dc_block.SetCycle(ti);
         paraview_dc_block.SetTime(t);
         paraview_dc_block.Save();

         paraview_dc_cylinder.SetCycle(ti);
         paraview_dc_cylinder.SetTime(t);
         paraview_dc_cylinder.Save();
      }
   }
