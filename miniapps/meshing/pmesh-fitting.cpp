...
#include "biv_ls_ls.hpp"   // GENERATED by Output/voxel-study/make_levelset.py

using namespace mfem;
using namespace std;

// -slstype 7: a level set sampled on a Cartesian grid and read from a file,
// for geometry nobody has a formula for -- a segmentation, or the staircase
// boundary of a voxelised mesh. File-scope so it can be reached through a plain
// FunctionCoefficient callback, like every other level set here.
static SampledLevelSet sampled_ls;
static real_t sampled_level_set(const Vector &x) { return sampled_ls.Eval(x); }

// "2 3 4" -> {2,3,4}. Empty string -> empty.
static void ParseAttrList(const char *str, Array<int> &out)
{
   out.SetSize(0);
   istringstream is(str);
   int a;
   while (is >> a) { out.Append(a); }
}

// "1:z 5:xy 7:all" -> per-attribute masks of components to hold fixed.
// Returns a map from boundary attribute to a 3-bit mask (bit d = fix component d).
static void ParseFixList(const char *str, std::map<int, int> &out)
{
   out.clear();
   istringstream is(str);
   string tok;
   while (is >> tok)
   {
      ...
      MFEM_VERIFY(c != string::npos && c > 0,
                  "-fixattr entry \"" << tok << "\" is not <attr>:<components>");
      const int attr = stoi(tok.substr(0, c));
      const string comp = tok.substr(c + 1);
      int mask = 0;
      if (comp == "all") { mask = 7; }
      else
      {
         for (char ch : comp)
         {
            ...
            else { MFEM_ABORT("-fixattr: unknown component '" << ch << "'"); }
         }
      }
      out[attr] |= mask;
   }
...
int main (int argc, char *argv[])
{
   ...
   const char *ls_file    = "";
   const char *fit_attr_s = "";
   const char *fix_attr_s = "";
   const char *out_pfx    = "";
   real_t bg_elem_size    = -1.0;
   int bg_ls_order        = -1;
   real_t bg_pad          = 0.2;
   real_t sf_rel_change   = -1.0;
   int sf_adapt_count     = -1;
   ...
   args.AddOption(&surf_ls_type, "-slstype", "--surf-ls-type",
                  ...
                  "6 - CSG cube/cyl/sphere, 7 - biventricular CSG (biv_ls_ls.hpp),\n\t"
                  "8 - sampled from a file (-lsf).");
   ...
   args.AddOption(&ls_file, "-lsf", "--level-set-file",
                  "Level set sampled on a Cartesian grid (implies -slstype 8). "
                  "Written by mesh_to_sdf.py.");
   args.AddOption(&fit_attr_s, "-fitattr", "--fit-boundary-attributes",
                  "Boundary attributes to fit to the zero level set, e.g. "
                  "\"2 3 4\". Generalises -smtype to several surfaces.");
   args.AddOption(&fix_attr_s, "-fixattr", "--fix-boundary-attributes",
                  "Node components to hold fixed, per boundary attribute, e.g. "
                  "\"1:z 5:all\". Replaces the -bnd 1/2/3/4 convention, which "
                  "keys the constraint to the attribute NUMBER and so cannot be "
                  "used on a mesh whose attributes name anatomical surfaces.");
   args.AddOption(&out_pfx, "-out", "--output-prefix",
                  "Write <prefix>.mesh and a ParaView collection of the result.");
   args.AddOption(&bg_elem_size, "-bgh", "--bg-element-size",
                  "Initial background mesh element size, in physical units. "
                  "The default background mesh is 4x4x4 over the whole bounding "
                  "box, and -bgamriter refines it around the zero level set -- "
                  "but that refinement only fires on elements the level set is "
                  "SEEN to cut, so a feature thinner than a starting element is "
                  "never detected and never refined. Set this below the thinnest "
                  "feature of the geometry.");
   args.AddOption(&sf_rel_change, "-sfrc", "--surf-fit-rel-change",
                  "Relative decrease in average fitting error, per iteration, "
                  "below which -sfa raises the fitting weight (TMOP default "
                  "0.001). Progress slower than the penalty needs but faster "
                  "than this threshold never triggers the ramp, so the solve "
                  "creeps at the initial weight until it runs out of "
                  "iterations. Raise it (e.g. 0.02) to make the ramp fire.");
   args.AddOption(&sf_adapt_count, "-sfac", "--surf-fit-adapt-count",
                  "Consecutive weight increases allowed before the solver "
                  "stops (TMOP default 10).");
   args.AddOption(&bg_pad, "-bgpad", "--bg-padding",
                  "Background mesh margin beyond the bounding box, as a "
                  "fraction of each side (default 0.2). The mesh only moves by "
                  "about one element during fitting, so the default margin "
                  "makes the background mesh 2.7x bigger than it needs to be.");
   args.AddOption(&bg_ls_order, "-bglo", "--bg-level-set-order",
                  "Polynomial order of the level set on the background mesh "
                  "(default: mesh order + 1). Order 2 is the minimum: TMOP reads "
                  "the Hessian, which is zero for a trilinear field.");
   args.Parse();
   if (!args.Good())
   {
   ...
   if (strlen(ls_file) > 0) { surf_ls_type = 8; }
   if (myid == 0) { args.PrintOptions(cout); }

   Array<int> fit_attrs;
   ParseAttrList(fit_attr_s, fit_attrs);
   std::map<int, int> fix_attrs;
   ParseFixList(fix_attr_s, fix_attrs);
   const bool explicit_bc = (fit_attrs.Size() > 0 || !fix_attrs.empty());

   Device device(devopt);
   if (myid == 0) { device.Print();}

#ifdef HYPRE_USING_GPU
   // hypre built with GPU support cannot run this miniapp's solvers on the
   // device, but it is fine on the host: MFEM points hypre at host memory when
   // the MFEM device is the CPU. Only bail out if a device backend is active.
   if (Device::Allows(Backend::DEVICE_MASK))
   {
      if (myid == 0)
      {
         cout << "\nThis miniapp is NOT supported with the GPU version of "
              << "hypre. Re-run with -d cpu.\n\n";
      }
   ...
   else if (surf_ls_type == 7) // analytic biventricular CSG
   {
      ls_coeff = new FunctionCoefficient(biv::LevelSet);
   }
   else if (surf_ls_type == 8) // sampled on a Cartesian grid, from a file
   {
      MFEM_VERIFY(strlen(ls_file) > 0, "-slstype 8 needs -lsf <file>.");
      sampled_ls.Load(ls_file);
      Vector bb_lo, bb_hi;
      sampled_ls.GetBoundingBox(bb_lo, bb_hi);
      if (myid == 0)
      {
         const int *nn = sampled_ls.Dims();
         cout << "\nSampled level set " << ls_file << ": "
              << nn[0] << " x " << nn[1] << " x " << nn[2] << " at h = "
              << sampled_ls.Spacing() << ", phi in [" << sampled_ls.Min()
              << ", " << sampled_ls.Max() << "]\n"
              << "  box [" << bb_lo(0) << ", " << bb_hi(0) << "] x ["
              << bb_lo(1) << ", " << bb_hi(1) << "] x ["
              << bb_lo(2) << ", " << bb_hi(2) << "]" << endl;
      }
      ls_coeff = new FunctionCoefficient(sampled_level_set);
   }
   else
   {
   ...
   if (surf_bg_mesh)
   {
      // The background mesh is built on the unit cube here and stretched onto
      // the bounding box further down, so the element counts come from the
      // requested size measured against that box (plus the 20% margin added
      // there on each side).
      int bgn[3] = {4, 4, 4};
      if (bg_elem_size > 0.0)
      {
         ...
         for (int d = 0; d < dim; d++)
         {
            const real_t len = (1.0 + 2.0*bg_pad) * (p_max(d) - p_min(d));
            bgn[d] = max(2, (int)std::ceil(len / bg_elem_size));
         }
         if (myid == 0)
         {
            cout << "Background mesh: " << bgn[0] << " x " << bgn[1];
            if (dim == 3) { cout << " x " << bgn[2]; }
            cout << " elements of about " << bg_elem_size
                 << " before AMR, margin " << 100*bg_pad << "% per side."
                 << endl;
         }
      }
      ...
      if (dim == 2)
      {
         mesh_surf_fit_bg = new Mesh(
            Mesh::MakeCartesian2D(bgn[0], bgn[1], Element::QUADRILATERAL, true));
      }
      else if (dim == 3)
      {
         mesh_surf_fit_bg = new Mesh(
            Mesh::MakeCartesian3D(bgn[0], bgn[1], bgn[2], Element::HEXAHEDRON,
                                  1.0, 1.0, 1.0, false));
      }
   ...
   if (surf_bg_mesh)
   {
      ...
      // SetCurvature above defaults to Ordering::byVDIM, so the components of a
      // node are NOT num_nodes apart. Indexing this as byNODES stretches each
      // entry by the wrong axis's length and silently produces a scrambled
      // background mesh -- harmless on a cube, wrong on anything else.
      const int bg_ordering = pmesh_surf_fit_bg->GetNodalFESpace()->GetOrdering();
      for (int i = 0; i < num_nodes; i++)
      {
         for (int d = 0; d < dim; d++)
         {
            const int idx = (bg_ordering == Ordering::byNODES)
                            ? i + d*num_nodes : i*dim + d;
            real_t length_d = p_max(d) - p_min(d),
                   extra_d = bg_pad * length_d;
            x_bg(idx) = p_min(d) - extra_d +
                        x_bg(idx) * (length_d + 2*extra_d);
         }
      }
      {
         Vector b_min(dim), b_max(dim);
         pmesh_surf_fit_bg->GetBoundingBox(b_min, b_max);
         if (myid == 0)
         {
            cout << "Background mesh box:";
            for (int d = 0; d < dim; d++)
            {
               cout << " [" << b_min(d) << ", " << b_max(d) << "]";
            }
            cout << "\n              mesh box:";
            for (int d = 0; d < dim; d++)
            {
               cout << " [" << p_min(d) << ", " << p_max(d) << "]";
            }
            cout << endl;
         }
         for (int d = 0; d < dim; d++)
         {
            MFEM_VERIFY(b_min(d) <= p_min(d) + 1e-10 &&
                        b_max(d) >= p_max(d) - 1e-10,
                        "background mesh does not contain the mesh in direction "
                        << d << ": [" << b_min(d) << ", " << b_max(d)
                        << "] vs [" << p_min(d) << ", " << p_max(d) << "]");
         }
      }
      const int bglo = (bg_ls_order > 0) ? bg_ls_order : mesh_poly_deg + 1;
      MFEM_VERIFY(bglo >= 2, "-bglo must be at least 2: TMOP reads the Hessian "
                  "of the background level set.");
      surf_fit_bg_fec = new H1_FECollection(bglo, dim);
   ...
   if (surface_fit_const > 0.0)
   {
      ...
      if (surf_bg_mesh)
      {
         ...
         {
            long bg_ne = pmesh_surf_fit_bg->GetNE();
            MPI_Allreduce(MPI_IN_PLACE, &bg_ne, 1, MPI_LONG, MPI_SUM,
                          MPI_COMM_WORLD);
            const long bg_nd = surf_fit_bg_fes->GlobalTrueVSize();
            if (myid == 0)
            {
               cout << "Background level set: " << bg_ne << " elements, "
                    << bg_nd << " dofs at order "
                    << surf_fit_bg_fec->GetOrder() << " ("
                    << 13.0 * bg_nd * sizeof(real_t) / 1e6
                    << " MB with grad + Hessian)." << endl;
            }
         }
      ...
      if (marking_type == 0 && fit_attrs.Size() == 0)
      {
      ...
      // Strategy 2: Mark all boundaries whose attribute is in the fit set.
      // -smtype names one attribute; -fitattr names several, which is what a
      // real geometry needs -- an LV endocardium, an RV endocardium and an
      // epicardium are three attributes but one zero isosurface.
      else if (marking_type > 0 || fit_attrs.Size() > 0)
      {
         Array<int> fit_set(fit_attrs);
         if (marking_type > 0) { fit_set.Append(marking_type); }
         fit_set.Sort();
         fit_set.Unique();
         int n_marked_be = 0;
         for (int i = 0; i < pmesh->GetNBE(); i++)
         {
            ...
            if (fit_set.Find(attr) < 0) { continue; }
            n_marked_be++;
         ...
         long n_be_g = n_marked_be, n_be_tot = pmesh->GetNBE();
         MPI_Allreduce(MPI_IN_PLACE, &n_be_g, 1, MPI_LONG, MPI_SUM, MPI_COMM_WORLD);
         MPI_Allreduce(MPI_IN_PLACE, &n_be_tot, 1, MPI_LONG, MPI_SUM, MPI_COMM_WORLD);
         if (myid == 0)
         {
            cout << "Fitting boundary attributes";
            for (int i = 0; i < fit_set.Size(); i++) { cout << " " << fit_set[i]; }
            cout << ": " << n_be_g << " of " << n_be_tot
                 << " boundary elements marked." << endl;
         }
      }
   ...
   if (explicit_bc)
   {
      // Per-attribute component constraints from -fixattr. A node shared by a
      // fitted surface and a fixed one keeps both: it slides along the fixed
      // plane while the fitting term pulls its free components onto the zero
      // level set. That is what keeps the base rim on the base.
      Array<int> ess_vdofs;
      for (int i = 0; i < pmesh->GetNBE(); i++)
      {
         const int attr = pmesh->GetBdrElement(i)->GetAttribute();
         auto it = fix_attrs.find(attr);
         if (it == fix_attrs.end()) { continue; }
         ...
         for (int d = 0; d < dim; d++)
         {
            ...
            for (int j = 0; j < nd; j++) { ess_vdofs.Append(vdofs[j + d*nd]); }
         }
      }
      ess_vdofs.Sort();
      ess_vdofs.Unique();
      long n_ess = ess_vdofs.Size();
      MPI_Allreduce(MPI_IN_PLACE, &n_ess, 1, MPI_LONG, MPI_SUM, MPI_COMM_WORLD);
      if (myid == 0)
      {
         cout << "Fixed node components from -fixattr: " << n_ess
              << " vdofs (sum over ranks)." << endl;
      }
      a.SetEssentialVDofs(ess_vdofs);
   }
   else if (move_bnd == false)
   {
   ...
   if (sf_rel_change >= 0.0)
   {
      solver.SetAdaptiveSurfaceFittingRelativeChangeThreshold(sf_rel_change);
   }
   if (sf_adapt_count > 0)
   {
      solver.SetMaxNumberofIncrementsForAdaptiveFitting(sf_adapt_count);
   }
   ...
   {
      ostringstream mesh_name;
      if (strlen(out_pfx) > 0) { mesh_name << out_pfx << ".mesh"; }
      else                     { mesh_name << "optimized.mesh"; }
      ofstream mesh_ofs(mesh_name.str().c_str());
      mesh_ofs.precision(16);
      pmesh->PrintAsSerial(mesh_ofs);
      if (myid == 0) { cout << "Wrote " << mesh_name.str() << endl; }
   }
   ...
   // ParaView export. node_shift is how far the repair moved each node: Warp By
   // Vector on it, scaled up, shows the staircase being pulled onto the surface.
   if (strlen(out_pfx) > 0)
   {
      ParGridFunction shift(pfespace);
      shift = x;
      shift -= x0;

      ParaViewDataCollection pv(out_pfx, pmesh);
      pv.SetPrefixPath("ParaView");
      pv.SetLevelsOfDetail(mesh_poly_deg);
      pv.SetHighOrderOutput(mesh_poly_deg > 1);
      pv.SetDataFormat(VTKFormat::BINARY);
      pv.RegisterField("node_shift", &shift);
      if (surface_fit_const > 0.0)
      {
         pv.RegisterField("level_set", &surf_fit_gf0);
         pv.RegisterField("fit_marker", &surf_fit_mat_gf);
      }
      pv.SetCycle(0);
      pv.SetTime(0.0);
      pv.Save();
      if (myid == 0)
      {
         cout << "Wrote ParaView/" << out_pfx << endl;
      }
   }
