...
int main(int argc, char *argv[])
{
   ...
   real_t dt = 1.0e-2;
   // Problem
   bool temperature_dependent_materials = true;
   // Postprocessing
   bool visit = false;
   bool paraview = true;
   ...
   args.AddOption(&dt, "-dt", "--time-step",
                  "Time step.");
   args.AddOption(&temperature_dependent_materials, "-tdm", "--temp-dep-materials",
                  "-no-tdm", "--no-temp-dep-materials",
   ...
   *nodes *= scale; // scale to meters
   ...
   if (!temperature_dependent_materials)
   {
   ...
   else
   {
      ...
      auto k_func = [k_val, Tref](real_t T, DenseMatrix &K)
      {
         int dim = K.Height();
         K = 0.0;
         for (int d=0; d<dim; d++) { K(d,d) = k_val * (1.0 - 0.0005 * (T - Tref)); }
      };
   ...
   // Set gf for temperature dependence if needed
   if (temperature_dependent_materials)
   {
   ...
   ///<--- Create gfs for material properties (just for visualization)
   // Just for heat capacity so we can just use a scalar GridFunction
   ...
   // NOTE: for temperature-dependent materials, this needs to be set before Setup()
   // Otherwise the initial temperature will be zero and material properties
   // will be incorrect at the first time step
   // Unless UpdatedParameters() is called after Setup() to force re-assembly
   real_t T0val = CelsiusToKelvin(37.0);
   ConstantCoefficient T0(T0val);
   T_gf.ProjectCoefficient(T0);
   Heat.SetInitialTemperature(T_gf);

   c_gf.ProjectCoefficient(*c);

   ///<--- Setup the solver (assemble matrices, etc.)
   Heat.EnablePA(pa);
   Heat.Setup();
   ...
   for (int step = 1; !last_step; step++)
   {
      ...
      ///<--- Project temperature-dependent material properties for visualization
      c_gf.ProjectCoefficient(*c);

      ///<--- Flag that parameters have been updated (e.g. for temperature-dependent coefficients)
      // This does not trigger re-assembly of the operators, but will do so at the next time step
      if (temperature_dependent_materials)
         Heat.UpdatedParameters();

      ///<--- Write fields to disk for VisIt/ParaView
