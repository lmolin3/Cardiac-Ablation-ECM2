    ...
    */
    ...
    public:
      ...
      PerfusionCoefficient(GridFunction *T_gf_,
          ...
          : T_gf(T_gf_), rho_b(rho_b_), c_b(c_b_), w_b_baseline(w_b_baseline_),
            correction_coeff(correction_coeff_), T_blood_core(T_blood_core_),
            rate_type(PerfusionRateType::CONSTANT)
      {
        ...
        w_b_func = [this](real_t /*damage*/) { return w_b_baseline; };
      }
      ...
      PerfusionCoefficient(GridFunction *T_gf_,
                           ...
                           real_t correction_coeff_ = 0.8,
                           real_t T_blood_core_ = 310.15)
          : T_gf(T_gf_), rho_b(rho_b_), c_b(c_b_), Damage_gf(Damage_gf_),
            rate_type(rate_type_), w_b_baseline(w_b_baseline_),
            correction_coeff(correction_coeff_), T_blood_core(T_blood_core_)
      {
        MFEM_ASSERT(T_gf != nullptr, "Temperature GridFunction must be provided for PerfusionCoefficient.");
        ...
        switch (rate_type)
        {
          ...
          case PerfusionRateType::PIECEWISE:
            w_b_func = [this](real_t damage) {
              real_t d = 1.0 - damage;
              return (d > damage_threshold) ? 0.0 : w_b_baseline;
            };
            break;
          case PerfusionRateType::NONLINEAR:
            w_b_func = [this](real_t damage) {
              if (damage <= 0.0)
                ...
                return w_b_baseline * exp(-damage);
            };
            break;
        }
      }
