  ...
  namespace common_ecm2
  {
    ...
    */
    ...
    protected:
      ...
      GridFunction *gf = nullptr;  //< Temperature GridFunction, NOT OWNED
    ...
    public:
      ...
      PerfusionCoefficient(GridFunction *gf_,
                           real_t rho_b_,
                           real_t c_b_,
      ...
      {
        MFEM_ASSERT(gf != nullptr, "Temperature GridFunction must be provided for PerfusionCoefficient.");
      ...
      {
        real_t T_local = gf->GetValue(T, ip);
    ...
    /** @brief Generic GridFunction-Dependent Coefficient
     ...
     *
     * The user provides a function that defines how the coefficient varies with a provided GridFunction.
     *
     * Example usage:
     *
     * ```cpp
     * auto conductivity_func = [](real_t u) {
     *     if (u < 310.15) // Below threshold
     *         return 0.5; 
     *     else
     *         return 0.4;
     * };
     *
     * GridFunctionDependentFunctionCoefficient k_coeff(&gf, conductivity_func);
     * ```
     */
    class GridFunctionDependentFunctionCoefficient : public Coefficient
    {
    protected:
      const GridFunction *gf;                       //< Temperature GridFunction, NOT OWNED
      std::function<real_t(real_t)> coeff_func;       //< User-defined function for temperature dependence
    public:
      GridFunctionDependentFunctionCoefficient(const GridFunction *gf_,
                                      std::function<real_t(real_t)> coeff_func_)
          : gf(gf_), coeff_func(coeff_func_)
      {}


      // Overload constructor without GridFunction (provided later)
      GridFunctionDependentFunctionCoefficient(std::function<real_t(real_t)> coeff_func_)
          : gf(nullptr), coeff_func(coeff_func_)
      {}

      real_t Eval(ElementTransformation &T, const IntegrationPoint &ip) override
      {
        MFEM_ASSERT(gf != nullptr, "GridFunction must be provided for GridFunctionDependentFunctionCoefficient.");
        real_t T_local = gf->GetValue(T, ip);
     ...
     *  Useful for defining material properties (e.g., thermal conductivity, specific heat, electrical conductivity) that vary with temperature.
     *
     * The user provides a function that defines how the coefficient varies with the providedgf.
     *
     * Example usage:
     *
     * ```cpp
     * auto conductivity_matrix_func = [](real_t T, DenseMatrix &K) {
     ...
     *    }
     * };
     * GridFunctionDependentMatrixFunctionCoefficient k_coeff(&gf, conductivity_matrix_func);
     * ```
     */
    class GridFunctionDependentMatrixFunctionCoefficient : public MatrixCoefficient
    {
    protected:
      const GridFunction *gf;                       //< Temperature GridFunction, NOT OWNED
      std::function<void(real_t, DenseMatrix &)> coeff_func; //< User-defined function for temperature dependence
    public:
      GridFunctionDependentMatrixFunctionCoefficient(int dim,
                                            const GridFunction *gf_,
                                            std::function<void(real_t, DenseMatrix &)> coeff_func_)
          : MatrixCoefficient(dim), gf(gf_), coeff_func(coeff_func_)
      {
      }
      ...
      GridFunctionDependentMatrixFunctionCoefficient(int dim,
                                            std::function<void(real_t, DenseMatrix &)> coeff_func_)
          : MatrixCoefficient(dim), gf(nullptr), coeff_func(coeff_func_)
      {
      }
      ...
      {
        MFEM_ASSERT(gf != nullptr, "Temperature GridFunction must be provided for GridFunctionDependentMatrixFunctionCoefficient.");
